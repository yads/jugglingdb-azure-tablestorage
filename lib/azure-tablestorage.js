...
AzureTablestore.prototype.destroyAll = function destroyAll(model, callback) {
    console.warn('This operation can be very slow and unreliable. To delete the table use schema.adapter.destroyTable(modelName). Note, this table may take some time to delete.');
    var tableQuery = new TableQuery().select(['RowKey', 'PartitionKey']);
    var tableName = this.tableName(model);
    var self = this;
    this.client.queryEntities(tableName, tableQuery, null, function(error, entities){
        if (error) {
            return callback(error);
        }

        async.each(entities.entries, function(item, cb) {
            var descriptor = {
                RowKey: item.RowKey,
                PartitionKey: item.PartitionKey
            };

            self.client.deleteEntity(tableName, descriptor, cb);

        }, callback);
    });
};

AzureTablestore.prototype.destroyTable = function destroyTable(model, callback) {
